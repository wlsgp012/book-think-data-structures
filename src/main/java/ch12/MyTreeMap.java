...
public class MyTreeMap<K, V> implements Map<K, V> {
    ...
    protected class Node {
        ...
        Node find(Comparable<? super K> k) {
            int compare = k.compareTo(key);
            if (compare > 0) {
                return Optional.ofNullable(right).map(r -> r.find(k)).orElse(null);
            } else if (compare < 0) {
                return Optional.ofNullable(left).map(l -> l.find(k)).orElse(null);
            } else {
                return this;
            }
        }
